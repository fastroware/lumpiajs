...
export async function serveProject() {
    ...
    try {
        // Load User Routes dynamically
        // Because of ESM, we need file:// URL
        ...
        const server = http.createServer(async (req, res) => {
            ...
            // 1. Static Files (Public) - skipped for now, add 'public' folder support if needed
            ...
            if (match) {
                try {
                    ...
                    const controllerPath = path.join(root, 'app', 'kontroler', controllerName + '.js');
                    
                    if (!fs.existsSync(controllerPath)) throw new Error(`Kontroler ${controllerName} ora ketemu!`);
                    
                    const module = await import('file://' + controllerPath + '?update=' + Date.now()); // cache busting
                    ...
                    const result = await instance[methodName](); // Call method (await support)
